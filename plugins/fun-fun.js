...
if (!texto) return conn.reply(m.chat, `ꕥ Por favor, ingrese un texto luego del número para hacer un Top *cantidad texto*`, m)
...
if (command == 'sorteo') {
if (!text) return conn.reply(m.chat, `ꕥ Por favor ingresa lo que deseas sortear.`, m)
let cantidad = 1
let premio = text
if (!isNaN(parseInt(args[0]))) {
cantidad = Math.min(Math.max(parseInt(args[0]), 1), 10)
...
if (!premio) return conn.reply(m.chat, `ꕥ Por favor, ingrese un texto luego del número para hacer el sorteo.`, m)
...
if (!text) return conn.reply(m.chat, `ꕥ Escribe tu nombre y el nombre de la otra personas para calcular su amor.`, m)
...
user.afk = Date.now()
user.afkReason = text
return await conn.reply(m.chat, `ꕥ *El Usuario ${await conn.getName(m.sender)} Estará AFK*\n○ *Motivo${text ? ': ' + text : ': Sin Especificar!'}*`, m)
}
...
if (!userId) return conn.reply(m.chat, `ꕥ Por favor, ingrese el nombre de alguna persona.`, m)
...
if (!userId) return conn.reply(m.chat, `ꕥ Por favor, ingrese el tag de algún usuario o responda a un mensaje.`, m)
